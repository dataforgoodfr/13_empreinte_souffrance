...
export default async function WFIArticleSection() {
  ...
  return (
    <section className=" h-screen p-8 sm:px-16 bg-[#E7E4FF] flex flex-col gap-5">
      <h1 className="w-full text-3xl sm:text-3xl lg:text-5xl font-bold text-[#3b0a0a] text-left">
        {scopedT('title.part1')}
      ...
      <figure className="w-full flex items-center justify-between gap-12">
        <Image src="/wfi_article_figure.png" width={560} height={620} className="block" alt="Picture of a WFI researchers" />
        <figcaption className="max-w-xl space-y-6">
          <p className="text-2xl text-[#3b0a0a]">
            {scopedT('description.part0')}
            &nbsp;<strong>{scopedT('description.strong1')}</strong>
            &nbsp;{scopedT('description.part1')}<br />
            <br/>
      ...
      <article className= "flex justify-between text-[#3b0a0a] font-extrabold tracking-wide">
        <hgroup>
        ...
        <hgroup>
          <h2 className="bg-white text-center text-3xl sm:text-4xl flex justify-start items-center w-fit ">4 {scopedT('key_number.years')}</h2>
